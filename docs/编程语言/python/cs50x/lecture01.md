...
- 在终端窗口中，您可以执行命令。要运行此程序，您需要将光标移到屏幕底部，单击终端窗口。现在您可以在终端窗口中输入第二个命令。在`$`符号旁边，输入`python hello.py`并按下键盘上的回车键。
- 回想一下，计算机实际上只理解0和1。因此，当您运行 `python hello.py`时，python 将解释您在`hello.py`中创建的文本，并将其翻译成计算机可以理解的0和1。
- 程序`python hello.py`运行的结果是`hello, world`。
- 恭喜！您刚刚创建了您的第一个程序。
...
- 我们编写程序时候不可避免的会产生bug。这些都是错误，是需要您解决的问题！不要气馁！这是成为优秀程序员过程的一部分。
- 想象一下，在我们的`hello.py`程序中，`print("hello, world")`我们输入了 `print("hello, world"`，而忽略了编译器要求的最后的 `)`。如果我故意犯了这个错误，你会发现编译器会在终端窗口中输出错误！
- 通常，错误消息会告知您错误并为您提供修复方法的线索。但是，很多时候编译器并不是这样友好。

# 改进您的第一个Python程序

- 我们可以个性化您的第一个 Python 程序。
- 在`hello.py`文件中，我们可以添加另一个函数。`input`是一个可以接受提示作为参数的函数。我们可以编辑代码如下：
  ```apache
...
- 注释是程序员跟踪他们在程序中所做的事情的一种方式，甚至可以告知其他人作者对代码块的意图。简而言之，它们是您自己和其他将看到您的代码的人的注释！
...
现在结果是正确的。 使用的`int(x)`称为“强制转换”，即将值临时从一种类型的变量（本例中为字符串）更改为另一种类型的变量（本例中为整数）。
...
此更改允许您的用户输入`1.2`和`3.4`输出结果总数是`4.6`.

- 但是，让我们想象一下，您想将总数四舍五入为最接近的整数。查看 Python 文档中的`round`，您将看到可用的参数是`round(number[n, ndigits])`。这些方括号表示程序员可以指定一些可选内容。因此，您可以通过`round(n)`将数字四舍五入为最接近的整数。或者，您可以按如下方式编写代码：
...
在这里，在第一行中，您正在创建`hello`函数。但是，这一次，您告诉编译器此函数只接受一个参数：一个名为`to`的变量。因此，当您调用`hello(name)`计算机会将`name`传递给`hello`函数中的`to`。这就是我们将值传递给函数的方式。非常有用！在终端窗口中运行`python hello.py`，您会看到输出更接近我们在本讲座前面提出的理想值。
